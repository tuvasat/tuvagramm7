...
def get_initial_and_is_change(word: str, endings_all: list, set_func) -> tuple:
    for endings in endings_all:
        ending_len = len(endings[0])
        endings_index = 1 + endings_all.index(endings)
        initial_word = word[:-ending_len]
        if ((word[-ending_len:] in endings) and
                (word == set_func(initial_word, endings_index))):
            return initial_word, endings_index
    return word, False
...
def string_to_words(in_string: str) -> list:
    ...
    и возвращает множество,
    состоящее из отдельных слов
